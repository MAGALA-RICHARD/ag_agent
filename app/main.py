...
@tool
def run_apsim_model(model: str, column: Union[str, list] ='Yield', lonlat: tuple = AUTO, start: int = 1986,
                    end: int = 2020, amount=AUTO, **kwargs):
    """
    Run an APSIM simulation and return results for a specified output column.
    ...
    column : str | list, default=Yield
        Name of the output/s column to extract from APSIM results. User may say run the maize model Yield with yield indicating the output
        if the user does not specify a column name, it implies an open prompt that requires giving them all the available simulated data. This is not ideal for production
    lonlat : tuple[float, float] default is AUTO
        Geographic coordinates representing the location of the weather data
        request. The tuple must be provided as (longitude, latitude).
        Example:
        (-93.62, 42.03)
    start : int
        ...
        2020
    amount : float
        Fertilizer amount to apply (50 kg/ha).

    Note: the start and end year is only time window for downloading weather data not the start and end of the simulations

    Returns
    -------
    list
        a list containing the simulation results for the requested column.

    Use this tool whenever the user asks to run an APSIM model simulation. The tool first changes the weather data
     in case the user provides a lonlat is not AUTO. If the user mentions a given area without lonlat use the location,
      use any lonlat location for that area, and report it.

    If the user mentions a crop model name like "Maize" or "Wheat",
    pass that name as the `model` argument.

    If the user provides a file path ending with ".apsimx",
    pass the full path as the `model` argument.

    Always return the specified output column from the APSIM results, unless if the users what something else like the mean, sum etc of the returned values

    Do not attempt to create your lonlat leave the lonlat argument to None if prompt does not say anything about weather or locations
    """

    model = apsimNGpy.ApsimModel(model)
    try:
        if lonlat is not AUTO:
            print(lonlat)
            fileName = f'{uuid.uuid4()}.met'
            model.get_weather_from_web(lonlat=lonlat, start=start, end=end, filename=fileName, source='daymet')
        # model.get_soil_from_web(lonlat=tuple(lonlat), **get_season_dates(lonlat), source='ssurgo')
        if amount is not AUTO:
            print(amount)
            model.edit_model(
        ...
        try:
            ...
            return {
                'columns': cols,
                'error': f'No such column: {column}. available columns are {cols}'
                         'tell the user what output variables are available'
            }

        return data.model.results.to_dict()
    except Exception as e:
        return {
            'success': False,
            'error': str(e),
            'suggestion': "give details of the error to the user",
            'message': "user needs to understand the details of the error"
        }
...
if __name__ == '__main__':
    ...
    openai_agent = ChatOpenAI(
        model="gpt-5-nano",temperature=0.8,
        api_key=api_key)
    agent = create_agent(
    ...
    st.title("APSIM AI Agent 🌱")
    # Default location (Ames, IA — relevant for your work)
    m = folium.Map(location=[42.0308, -93.6319], zoom_start=10)
        ...
        try:
            ...
            state = {
                "messages": [
                    {
                        "role": "user",
                        "content": f"Run APSIM simulation at lonlat=({lon, lat}),"
                    }
                ]
            }
